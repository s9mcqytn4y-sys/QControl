    ...
    suspend fun eksekusi(item: ItemOutboxSinkronisasi, token: String? = null): HasilOperasi<Unit> {
        ...
        return when (hasilRemote) {
            ...
            is HasilOperasi.Gagal -> {
                ...
                val statusHttp = (kesalahanRemote as? KesalahanAplikasi.Server)?.kode?.toIntOrNull()
                val kodeKesalahan = (kesalahanRemote as? KesalahanAplikasi.Server)?.kode

                if (kesalahanRemote is KesalahanAplikasi.Server && kesalahanRemote.kode == "409") {
                    // 4. Tandai konflik (tidak akan dicoba lagi otomatis)
                    val hasilTandaiKonflik = repositoriLokal.tandaiKonflik(
                        id = item.id,
                        pesan = kesalahanRemote.pesan,
                        lastHttpStatus = statusHttp,
                        lastErrorCode = kodeKesalahan
                    )
                    ...
                    val hasilTandaiGagal = repositoriLokal.tandaiGagal(
                        id = item.id,
                        pesan = kesalahanRemote.pesan,
                        lastHttpStatus = statusHttp,
                        lastErrorCode = kodeKesalahan
